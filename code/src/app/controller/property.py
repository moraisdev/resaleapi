...
class PostProperty(Resource):
    ...
    def post(self):
        ...
        parser.add_argument(
            "evaluated_value", type=float, required=True, location="json"
        )
        parser.add_argument(
            "property_value", type=float, required=True, location="json"
        )
...
class PutProperty(Resource):
    ...
    def put(self, property_id):
        ...
        try:
            mongo_connection().property.update_one(
                {"property_id": property_id}, {"$set": payload}
            )
            return Response("success", status=200, mimetype="application/json")
        except Exception as e:
            return Response(str(e), status=500, mimetype="application/json")
